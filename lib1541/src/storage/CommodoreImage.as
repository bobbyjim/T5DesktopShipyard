	...
	public class CommodoreImage
	{
        ...
        public function joinFile( chain:Array, numberOfBytes:int=254 ):LByteArray
        {
        	...
        	for each (var entry:Object in chain)
        	{
				...
				// RJE fix 2011-07-10
				var lengthToRead:uint = Math.min( numberOfBytes, sec.bytesAvailable );
				
        		if ( t == 0 ) // lsu
					lengthToRead = Math.min( s, sec.bytesAvailable );

       			sec.readBytes( result, result.length, lengthToRead );				
        	}
        	return result;
        }
		...
		public function setDirectoryChain( chain:Array ):Boolean
		{
			image[ 'dir-chain' ] = chain;
			return true;
		}
