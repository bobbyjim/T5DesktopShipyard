...
package storage
/*
	2011 version - updated to include the directory chain information.
...
*/
{
	...
	public class CMD extends CommodoreImage
	{
		...
		public var ZONES:Array          = [ [0,0], [0,0], [0,0], [0,0] ]; // TODO - use this to generate SECTORS_IN_TRACK
		public var TRACKS:int           = 0;
    	public var SECTORS_IN_TRACK:Array = [];
    	public var TOTAL_SECTORS:int    = 0;
		public var HEADER_TRACK:int     = 1;
		...
		public function readFile( filename:String, position:int, data:LByteArray ):String
		{
			return filename;
		}
		...
		override public function getImageDetails():String
		{
			...
			{
				a += "Zone " + i + "    " + ZONES[i][0] + " tracks, " + ZONES[i][1] + " sectors per track\n";
			}
 ...
 */		    
        ...
        public function readImage( bytes:ByteArray ):Boolean
        {
            ...
            this.setDirectoryChain( readFileData( DIRECTORY_TRACK, DIRECTORY_SECTOR, trackList, false ) );
		...
		/*
		  Bytes:$00-01: Track/Sector location of the first directory sector (should
            ...
            02: Disk DOS version type (see note below)
 				...
 				1541: offset = 0x90, verison = '2A'
		   		1581: offset = 0x04, version = '3D'
	  	8050 && 8250: offset = 0x06, version = '2C'
 				
		offset:
			+00-0F: Disk Name, $A0 padded
			+10-11: $A0
			+12-13: Disk ID
			   +14: $A0 
			+15-16: DOS Version 
...
*/
        ...
        public function readDir( trackList:Array ):Array
        {
    		...
    		var sectorList:Array = readFileData( DIRECTORY_TRACK, DIRECTORY_SECTOR, trackList );
            ...
            for each (var entry:Object in sectorList)
            {
                ...
                for ( var offset:int = 0; offset < 256; offset += 32 )
                {
                    ...
                    {
      			    	fileEntry = readFileData( firstTrack, firstSector, trackList );
        ...
        //
        public function readFileData( track:int, 
								  sector:int, 
                                  trackList:Array,
								  includeData:Boolean=true ):Array
		{
		...
		public function writeImageDirectory( image:LByteArray ):void
		{
			...
			for each (var entry:Object in dir)
			{
				...
				for ( var i:int=0; i<entryName.length; i++ )
				{
					directoryBuffer[ offset+0x05 + i ] = entryName.charCodeAt(i);
				}
				for ( var j:int=entryName.length; j<16; j++ )
				{
					directoryBuffer[ offset+0x05 + j ] = 0xa0;
				}
